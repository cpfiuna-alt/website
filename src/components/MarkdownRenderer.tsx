...
export const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ content, className = "" }) => {
  ...
  const formatMarkdown = (markdownText: string) => {
    ...
    // First, normalize line endings and trim whitespace
    const normalizedText = markdownText.replace(/\r\n/g, '\n').trim();
    
    // Convert headers with specific classes to override prose
    let formattedText = normalizedText
      .replace(/^# (.+)$/gm, '<h1 class="!text-3xl !font-bold !mb-4 !mt-6 !text-foreground">$1</h1>')
      .replace(/^## (.+)$/gm, '<h2 class="!text-2xl !font-bold !mb-3 !mt-6 !text-foreground">$1</h2>')
      .replace(/^### (.+)$/gm, '<h3 class="!text-xl !font-bold !mb-2 !mt-4 !text-foreground">$1</h3>')
    ...
    formattedText = formattedText
      ...
      .replace(/^(\d+)\. (.+)$/gm, '<li class="!ml-6 !list-decimal !text-muted-foreground">$2</li>');
    
    // Wrap lists
    formattedText = formattedText
      .replace(/(<li class="!ml-6 !list-disc !text-muted-foreground">.*?<\/li>(\n|$))+/gs, (match) => {
        return `<ul class="!mb-4 !space-y-2">${match}</ul>`;
      })
      .replace(/(<li class="!ml-6 !list-decimal !text-muted-foreground">.*?<\/li>(\n|$))+/gs, (match) => {
